    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ scale: 0.96, y: 16 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.96, opacity: 0 }}
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 18, justifyContent: 'flex-end' }}>
          ...
          <button onClick={() => onSave(form)} style={{ padding: '8px 18px', borderRadius: 9, border: 'none', background: `linear-gradient(135deg, ${D.accent}, #7B9FF5)`, color: '#fff', cursor: 'pointer', fontSize: 13, fontWeight: 600, boxShadow: '0 4px 16px rgba(79,110,247,0.3)' }}>Speichern</button>
        </div>
      </motion.div>
    </motion.div>, document.body
  );
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        background: isSelected ? 'rgba(79,110,247,0.08)' : hovered ? 'rgba(255,255,255,0.025)' : 'transparent',
...
export default function MyLeads() {
  ...
  const KPI_CARDS = [
    { label: 'Gesamt',          value: counts.alle,           color: D.accent,   glow: 'rgba(79,110,247,0.15)',   icon: PhoneCall },
    { label: 'Zu anrufen',      value: counts.anrufen,        color: D.orange, glow: 'rgba(79,110,247,0.15)',   icon: Phone },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', padding: '24px 28px 20px', boxSizing: 'border-box', gap: 16, overflow: 'hidden', position: 'relative' }}>
      ...
      <div style={{ position: 'fixed', inset: 0, pointerEvents: 'none', zIndex: 0 }}>
        <div style={{ position: 'absolute', top: '-5%', left: '30%', width: 400, height: 400, background: 'rgba(79,110,247,0.06)', borderRadius: '50%', filter: 'blur(100px)' }} />
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0, position: 'relative', zIndex: 1 }}>
        ...
        <motion.div style={{ display: 'flex', flexDirection: 'row', gap: 6, flexShrink: 0, alignItems: 'center' }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}>
          ...
          <button onClick={() => setShowScreenshot(true)} style={{
            ...
            border: `1px solid rgba(79,110,247,0.2)`, cursor: 'pointer', lineHeight: 1,
          }}>
          ...
          <button onClick={() => setShowAddLead(true)} style={{
            ...
            background: `linear-gradient(135deg, ${D.accent}, #7B9FF5)`,
            color: '#fff', boxShadow: '0 4px 16px rgba(79,110,247,0.3)',
          }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexShrink: 0, alignItems: 'center' }}>
                    ...
                    <div style={{ position: 'relative' }}
                      ...
                      <button onClick={() => navigate('/demo-wizard', { state: { lead: selectedLead } })} style={{
                        display: 'flex', alignItems: 'center', gap: 5, padding: '6px 11px', borderRadius: 8,
                        fontSize: 11.5, fontWeight: 600, background: 'rgba(79,110,247,0.12)', color: '#4F6EF7',
                        border: '1px solid rgba(79,110,247,0.2)', cursor: 'pointer',
                      }}>

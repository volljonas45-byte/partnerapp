      ...
      {options.map(opt => {
        ...
        return (
          <button key={opt.value} type="button" onClick={() => toggle(opt.value)}
            style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '7px 13px', borderRadius: '99px', border: `2px solid ${sel ? '#0071E3' : '#E5E5EA'}`, background: sel ? '#0071E3' : '#fff', color: sel ? '#fff' : '#3C3C43', fontSize: '13px', fontWeight: 500, cursor: 'pointer', transition: 'all 0.15s' }}>
            {opt.label}
          </button>
        );
      })}
      ...
      {options.map(opt => {
        ...
        return (
          <button key={opt.value} type="button" onClick={() => onChange(opt.value)}
            style={{ padding: '7px 14px', borderRadius: '99px', border: `2px solid ${sel ? '#0071E3' : '#E5E5EA'}`, background: sel ? '#0071E3' : '#fff', color: sel ? '#fff' : '#3C3C43', fontSize: '13px', fontWeight: 500, cursor: 'pointer', transition: 'all 0.15s' }}>
            {opt.label}
          </button>
        );
      })}
    ...
    <div style={{ minHeight: '100%', background: '#F5F5F7', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #F2F2F7', padding: '16px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <span style={{ fontSize: '15px', fontWeight: 600, color: '#1D1D1F' }}>Branding-Projekt einrichten</span>
        </div>
          ...
          {STEPS.map((s, i) => {
            ...
            return (
              <div key={s.key} style={{ display: 'flex', alignItems: 'center', flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px', fontWeight: active ? 600 : 400, color: done ? '#34C759' : active ? '#F59E0B' : '#C7C7CC' }}>
                  <div style={{ width: '22px', height: '22px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: done ? '#34C759' : active ? '#F59E0B' : '#F2F2F7', flexShrink: 0 }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        <div style={{ maxWidth: '600px', margin: '0 auto', padding: '32px 24px 48px' }}>
          ...
          {step === 1 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div>
                <h2 style={{ fontSize: '20px', fontWeight: 700, color: '#1D1D1F', margin: 0 }}>Branding-Details</h2>
                    ...
                    {teamMembers.map(m => {
                      ...
                      return (
                        <button key={m.id} type="button" onClick={() => set('assignee_id')(active ? null : m.id)}
                          style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 14px', borderRadius: '99px', border: `2px solid ${active ? m.color || '#F59E0B' : '#E5E7EB'}`, background: active ? (m.color || '#F59E0B') + '18' : '#fff', cursor: 'pointer', transition: 'all 0.15s' }}>
      ...
      <div style={{ background: '#fff', borderTop: '1px solid #F2F2F7', paddingTop: 16, paddingLeft: isMobile ? 20 : 32, paddingRight: isMobile ? 20 : 32, paddingBottom: isMobile ? 'calc(env(safe-area-inset-bottom) + 86px)' : 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
        ...
        {!isLastStep ? (
          <button type="button" onClick={next} disabled={!canNext()}
            style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '9px 20px', borderRadius: '10px', border: 'none', background: canNext() ? '#F59E0B' : '#E5E5EA', color: canNext() ? '#fff' : '#8E8E93', fontSize: '14px', fontWeight: 600, cursor: canNext() ? 'pointer' : 'not-allowed', transition: 'background 0.15s' }}>
            Weiter <ChevronRight size={16} />
          </button>
        ) : (
          <button type="button" onClick={handleCreate} disabled={loading}
            style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '9px 20px', borderRadius: '10px', border: 'none', background: loading ? '#E5E5EA' : '#F59E0B', color: loading ? '#8E8E93' : '#fff', fontSize: '14px', fontWeight: 600, cursor: loading ? 'not-allowed' : 'pointer' }}>
            {loading ? (
              <>

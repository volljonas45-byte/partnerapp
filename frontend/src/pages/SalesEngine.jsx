...
import {
  ...
  Plus, Clock, X, UserPlus, PhoneCall,
} from 'lucide-react';
...
function AddLeadModal({ onClose, existingLeadClientIds, onCreate }) {
  ...
  const available = useMemo(() => {
    const existing = new Set(existingLeadClientIds);
    return clients
      .filter(c => !existing.has(c.id))
      .filter(c => !search || c.company_name?.toLowerCase().includes(search.toLowerCase()) || c.contact_person?.toLowerCase().includes(search.toLowerCase()));
  }, [clients, existingLeadClientIds, search]);

  return (
    <div
      style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(6px)' }}
      onClick={onClose}
    >
      <div style={{ background: '#fff', borderRadius: 18, padding: '24px', width: '100%', maxWidth: 440, maxHeight: '70vh', display: 'flex', flexDirection: 'column', boxShadow: '0 24px 60px rgba(0,0,0,0.18)' }} onClick={e => e.stopPropagation()}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <span style={{ fontSize: 16, fontWeight: 700, color: '#1D1D1F' }}>Kunde als Lead importieren</span>
          </div>
        ...
        <div style={{ position: 'relative', marginBottom: 12 }}>
          ...
          <input
            ...
            style={{ width: '100%', padding: '9px 10px 9px 32px', borderRadius: 10, fontSize: 13.5, border: '1.5px solid #E5E5EA', outline: 'none', boxSizing: 'border-box' }}
          />
        </div>
        ...
        <div style={{ flex: 1, overflow: 'auto', minHeight: 0 }}>
          {available.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '32px 0', color: '#86868B', fontSize: 13 }}>
              {clients.length === 0 ? 'Keine Kunden vorhanden' : 'Alle Kunden sind bereits Leads'}
            </div>
          ) : (
            available.map(c => (
              <div
                ...
                style={{
                  ...
                  padding: '10px 12px', borderRadius: 10, cursor: 'pointer', transition: 'background 0.1s',
                }}
                onMouseEnter={e => e.currentTarget.style.background = '#F5F5F7'}
                onMouseLeave={e => e.currentTarget.style.background = ''}
              >
                <div>
                  <div style={{ fontSize: 14, fontWeight: 600, color: '#1D1D1F' }}>{c.company_name}</div>
                  <div style={{ fontSize: 12, color: '#86868B' }}>{[c.contact_person, c.phone].filter(Boolean).join(' · ')}</div>
                </div>
                <Plus size={16} color="#0071E3" />
              </div>
            ))
          )}
        </div>
...
export default function SalesEngine() {
  ...
  // All leads for client-id set (for add modal)
  const { data: allLeads = [] } = useQuery({ queryKey: ['sales-leads-all'], queryFn: () => salesApi.listLeads() });

  // Mutations
  const createLeadMut = useMutation({
    mutationFn: (client_id) => salesApi.createLead({ client_id }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['sales-leads'] }); toast.success('Lead erstellt'); setShowAddLead(false); },
  ...
  const updateCallMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['sales-stats'] }); qc.invalidateQueries({ queryKey: ['sales-leads'] }); qc.invalidateQueries({ queryKey: ['sales-calls-recent'] }); },
  });

  const updateLeadMut = useMutation({
    mutationFn: ({ id, data }) => salesApi.updateLead(id, data),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['sales-leads'] }); qc.invalidateQueries({ queryKey: ['sales-stats'] }); },
  });
  ...
  async function handleCall(lead) {
    try {
      ...
      setActiveCall({ callId: call.id, clientName: lead.company_name, phone: lead.phone, leadId: lead.id, clientId: lead.client_id });
  ...
  const existingLeadClientIds = useMemo(() => allLeads.map(l => l.client_id), [allLeads]);
  ...
  return (
    <div style={{ padding: '24px 28px', maxWidth: 1400, margin: '0 auto' }} className="animate-fade-in">
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              ...
              <button
                ...
                style={{
                  ...
                  fontSize: 12.5, fontWeight: 600, background: 'rgba(0,113,227,0.1)', color: '#0071E3',
            ...
            {leadsLoading ? (
              ...
              <div style={{ padding: '40px 0', textAlign: 'center' }}>
                ...
                <div style={{ fontSize: 12.5, color: '#86868B', marginTop: 3 }}>
                  {tab === 'due' ? 'Keine Follow-ups heute fällig' : 'Importiere Kunden als Leads'}
                </div>
              </div>
            ) : (
              leads.map(lead => (
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 14, fontWeight: 600, color: '#1D1D1F', letterSpacing: '-0.2px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{lead.company_name}</div>
                    <div style={{ fontSize: 12, color: '#86868B', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{[lead.contact_person, lead.phone].filter(Boolean).join(' · ')}</div>
                  </div>
      ...
      {showAddLead && <AddLeadModal existingLeadClientIds={existingLeadClientIds} onClose={() => setShowAddLead(false)} onCreate={clientId => createLeadMut.mutate(clientId)} />}

...
const KANAELE_OPTIONS = [
  { value: 'instagram', label: 'Instagram' },
  { value: 'facebook',  label: 'Facebook' },
  { value: 'linkedin',  label: 'LinkedIn' },
  { value: 'tiktok',    label: 'TikTok' },
  { value: 'youtube',   label: 'YouTube' },
  { value: 'xing',      label: 'Xing / X' },
];

const ZIEL_OPTIONS = [
  { value: 'reichweite', label: 'Reichweite aufbauen' },
  { value: 'leads',      label: 'Leads generieren' },
  { value: 'community',  label: 'Community aufbauen' },
  { value: 'branding',   label: 'Markenbekanntheit' },
  { value: 'verkaeufe',  label: 'Verkäufe steigern' },
...
export default function SocialMediaWizard() {
  ...
  const autoProjectName = () =>
    data.company_name
      ? `${data.company_name} – Social Media ${new Date().getFullYear()}`
      : '';
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (data.kanaele?.length) {
        const labels = data.kanaele.map(v => KANAELE_OPTIONS.find(o => o.value === v)?.label || v);
        descParts.push(`Kanäle: ${labels.join(', ')}`);
      }
      if (data.ziel) {
        const zielLabel = ZIEL_OPTIONS.find(o => o.value === data.ziel)?.label || data.ziel;
        descParts.push(`Hauptziel: ${zielLabel}`);
      }
          ...
          {step === 1 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <SectionCard title="HAUPTZIEL">
                <ChipGroup options={ZIEL_OPTIONS} value={data.ziel} onChange={set('ziel')} />
              </SectionCard>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <SectionCard title="PROJEKTNAME">
                <Field label="Name des Projekts">
                  <TextInput value={data.project_name || autoProjectName()} onChange={set('project_name')} placeholder={autoProjectName() || 'z.B. Muster GmbH – Social Media 2026'} />
                </Field>
              </SectionCard>
              <SectionCard title="BUDGET & NOTIZEN">
                <Field label="Projektwert (€) — optional">
                  <TextInput value={data.project_value} onChange={set('project_value')} type="number" placeholder="z.B. 1500" />
                </Field>
              ...
              {/* Summary */}
              <div style={{ padding: '16px', borderRadius: '14px', background: 'rgba(236,72,153,0.05)', border: '1px solid rgba(236,72,153,0.18)' }}>
                <div style={{ fontSize: '12px', fontWeight: 600, color: '#BE185D', marginBottom: '10px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Wird erstellt</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  {[
                    { label: 'Kunde',   value: data.company_name },
                    { label: 'Projekt', value: data.project_name || autoProjectName() },
                    data.kanaele?.length > 0 && { label: 'Kanäle',   value: `${data.kanaele.length} ausgewählt` },
                    data.ziel && { label: 'Ziel', value: ZIEL_OPTIONS.find(o => o.value === data.ziel)?.label || data.ziel },
